...
27. Не создавай ручные пользовательские или административные Telegram-команды. Все действия бота должны выполняться только через inline keyboard в каждом сообщении: «Создать ключ», «Мой ключ», «Проверить срок», «Продлить ключ», «Проверить привязку», «Отозвать ключ», «Помощь», «Назад» и «Обновить». Бот не является источником истины и всегда обращается к backend API.

28. Действие inline-кнопки «Создать ключ» должно быть идемпотентным. Если у пользователя уже есть активный ключ, не создавать новый и написать «Ключ уже создан», показать TTL. Параллельные нажатия не должны создать два ключа. Каждое сообщение, включая ошибку, должно иметь inline-кнопки; callback data не должна содержать ключи или HWID.
...
- административные inline-действия доступны только allowlist Telegram IDs и ролям;
...
### 22.4 Только inline-кнопки, без ручных команд

Бот не должен иметь пользовательских или административных текстовых команд. Пользователь не вводит `/create_key`, `/status`, `/renew` или другие команды вручную. Все действия выполняются через inline keyboard, которая прикреплена к каждому сообщению бота.
...
- каждое сообщение бота, включая ошибку, имеет inline-кнопки;
- после нажатия бот по возможности редактирует существующее сообщение, а не создаёт цепочку новых сообщений;
- после каждого результата снова показывать релевантное inline-меню;
- текстовый ввод пользователя не используется для выбора действия;
- неизвестный текст игнорировать или отвечать тем же главным меню;
- полные ключи не передавать в `callback_data`;
- `callback_data` содержит только короткий opaque action token;
- действие повторно проверяется backend по Telegram user ID, роли, nonce и TTL action token;
- опасные операции требуют второй inline-кнопки подтверждения;
- кнопка `Назад` возвращает в главное меню;
- кнопка `Обновить` получает один свежий status snapshot без фонового polling.

Административные действия также доступны только через отдельные inline-кнопки после проверки роли и Telegram ID. Отдельного ручного admin command interface не создавать.

Привязка к устройству выполняется только приложением через proof-of-possession. Бот не принимает raw HWID, serial, IMEI, MAC или текстовый fingerprint.
...
4. кнопка `Проверить срок` показывает правильный TTL.
...
### 27.1 Запрет ручных команд

Production-бот не должен публиковать и обрабатывать ручные команды для пользователя или администратора. Не добавлять command menu с `/my_key`, `/status`, `/create_key`, `/renew_key`, `/revoke_key` и `/admin/...`.

Технический `/start` может использоваться только Telegram для первичного открытия чата и сразу заменяется главным сообщением с inline keyboard. Никакого ручного command workflow после этого нет.

Бот должен обрабатывать только:

- inline callback actions;
- безопасный одноразовый deep-link token, если он нужен для onboarding;
- системные события backend.

### 27.2 Кнопки главного меню

Каждое сообщение должно содержать актуальную inline keyboard:

```text
[Создать ключ] [Мой ключ]
[Проверить срок] [Продлить ключ]
[Проверить привязку] [Отозвать ключ]
[Обновить] [Помощь]
```

После callback:

1. Telegram user ID и роль проверяются.
2. Callback token проверяется по nonce и TTL.
3. Backend выполняет ровно одно идемпотентное действие.
4. Бот редактирует текущее сообщение.
5. В ответе снова присутствуют inline-кнопки.
6. Секреты не помещаются в callback data.

### 27.3 Состояния кнопок

- `Создать ключ`: создать или вернуть `Ключ уже создан`;
- `Мой ключ`: показать masked key или одноразовый защищённый просмотр;
- `Проверить срок`: один status snapshot и countdown без фоновых запросов;
- `Продлить ключ`: запрос renewal только после server policy;
- `Проверить привязку`: показать только безопасный binding status;
- `Отозвать ключ`: сначала показать подтверждение, затем выполнить revoke;
- `Обновить`: один status-запрос по явному нажатию;
- `Помощь`: показать короткую справку и кнопку `Назад`.

Нажатия должны быть идемпотентными. Повторное нажатие `Создать ключ` не создаёт второй активный grant. Повторное нажатие `Отозвать ключ` не вызывает ошибку и не создаёт дополнительный revoke event.

### 27.4 Ошибки тоже с кнопками

Каждое сообщение об ошибке должно снова содержать keyboard:
...
Не отправлять пользователю traceback, HTTP body, internal reason code, HWID или server database ID.
