...
27. Не создавай ручные пользовательские или административные Telegram-команды. В каждом сообщении бота должны быть только две inline-кнопки: «Создать ключ» и «Мой ключ». Кнопка «Мой ключ» выводит всё: ключ, статус, TTL, binding status и masked device ID. Не добавляй кнопки или действия «Продлить», «Проверить привязку», «Отозвать», «Обновить», «Помощь» или «Проверить срок». В каждом сообщении обязательно писать: «По поводу помощи — @xvcey». Бот всегда обращается к backend API.

28. Действие inline-кнопки «Создать ключ» должно быть идемпотентным. Если у пользователя уже есть активный ключ, не создавать новый и написать «Ключ уже создан», показать всю информацию через тот же экран «Мой ключ». Параллельные нажатия не должны создать два ключа. Каждое сообщение, включая ошибку, должно иметь только эти две inline-кнопки; callback data не должна содержать ключи или HWID. После истечения новый grant может быть создан только повторным нажатием «Создать ключ» по server policy.
...
- в каждом сообщении показывать контакт помощи `По поводу помощи — @xvcey`.
...
### 22.4 Только две inline-кнопки

Бот не должен иметь ручных пользовательских или административных команд. В основном интерфейсе и в каждом ответе должны оставаться только две inline-кнопки:
...
Статус: Ключ истёк
Привязка: да
Истекает через: 00:00:00
```

Правила:

- не добавлять кнопки `Продлить`, `Проверить привязку`, `Отозвать`, `Обновить`, `Помощь`, `Назад` или `Проверить срок`;
...
- каждое сообщение, включая ошибку, снова содержит только эти две кнопки;
- в каждом сообщении писать: `По поводу помощи — @xvcey`;
...
### 22.7 Binding status внутри «Мой ключ»
...
2. Повторное нажатие `Создать ключ` возвращает `Ключ уже создан`.
...
В этом же сообщении снова находятся только две кнопки:
...
4. если активного grant нет или прежний grant истёк, создаёт новый grant по server policy;
5. выводит результат через экран `Мой ключ`;
6. снова показывает только две кнопки;
7. добавляет текст `По поводу помощи — @xvcey`.

Отдельного действия продления, отзыва, обновления, проверки привязки или помощи не создавать. Внутренний server-side revoke остаётся доступен backend policy, но не отображается в Telegram-боте.

### 27.4 Callback и повторные нажатия
...
- каждое сообщение, включая ошибку, содержит только две кнопки;
- каждое сообщение содержит `По поводу помощи — @xvcey`.
