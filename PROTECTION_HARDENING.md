...
27. Не создавай ручные пользовательские или административные Telegram-команды. В каждом сообщении бота должны быть только три inline-кнопки: «Создать ключ», «Мой ключ» и «Помощь». Кнопка «Мой ключ» выводит всё: имя ключа, сам ключ в безопасном виде, статус, TTL, binding status и masked device ID. Не добавляй кнопки или действия «Продлить», «Проверить привязку», «Отозвать», «Обновить», «Назад» или «Проверить срок». Текст «По поводу помощи — @xvcey» показывать только после нажатия кнопки «Помощь». Бот всегда обращается к backend API.
...
4. Backend возвращает `already_exists=true`, `key_id`, `key_name`, `status`, `expires_at`, binding status и masked device ID.
...
### 22.6 Имя ключа XVCEN

Каждому новому ключу backend присваивает отображаемое имя формата:
...
- постоянный prefix `XVCEN-`;
- ровно 5 случайных букв после prefix;
- использовать только заглавные латинские буквы `A-Z`;
- генерировать буквы через CSPRNG на backend;
- проверять уникальность `key_name` и повторять генерацию при коллизии;
- хранить `key_name` отдельно от настоящего license secret;
- показывать `key_name` в `Мой ключ`.

Пять букв — это только короткое имя для отображения. Они не являются секретом и не заменяют настоящий license secret. Настоящий секрет должен по-прежнему иметь не менее 256 бит энтропии, храниться в hash или KMS-encrypted envelope и не приниматься из callback data.

### 22.7 Формат license key

Ключ должен быть удобен для копирования и иметь достаточную энтропию:
...
1. Новый пользователь создаёт ключ с уникальным именем формата `XVCEN-ABCDE`.
...
4. если активного grant нет или прежний grant истёк, создаёт новый grant и новое имя формата `XVCEN-ABCDE` по server policy;
